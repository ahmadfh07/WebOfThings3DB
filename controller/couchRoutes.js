...
router.get("/sensor/:sensorId/:zoneId", async (req, res) => {
  try {
    ...
    res.send({ error: false, msg: "Sensor data retrieved", data: result.rows });
  } catch (err) {
...
router.post("/sensor/:sensorId/:zoneId", async (req, res) => {
  try {
    const newSensor = await Sensor.create({ sensor_id: req.params.sensorId, zone_id: req.params.zoneId, data: { location: { longitude: req.body.longitude, latitude: req.body.latitude }, value: req.body.value } });
